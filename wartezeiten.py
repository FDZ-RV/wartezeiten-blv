...
def load_var_df_and_create_status_6(data_path, berichtsjahr):
    ...
    index = pd.MultiIndex.from_product([
        df_kids["rv_id"].unique(), np.arange(start_year, end_year + 1), np.arange(start_month, end_month + 1)
    ], names=["rv_id", "JAHR", "MONAT"])

    # case "KBZ"
    df_kid_kbz = pd.Series(index=index, data=0, name="KBZ").reset_index()
    df_kid_kbz["monat_ad"] = df_kid_kbz["JAHR"] * 12 + df_kid_kbz[
        "MONAT"]  # nr of months from (0000,01) to (JAHR,MONAT)

    # case "KEZ"
    df_kid_kez = pd.Series(index=index, data=0, name="KEZ").reset_index()
    df_kid_kez["monat_ad"] = df_kid_kez["JAHR"] * 12 + df_kid_kez[
        "MONAT"]  # nr of months from (0000,01) to (JAHR,MONAT)
    ...
    for id_kid in range(1, max_n_kids + 1):
        ...
        df_kids[column] = df_kids[f"GBKIJ{id_kid}"].astype(float) * 12 + df_kids[f"GBKIM{id_kid}"]
    ...
    for id_kid in range(1, max_n_kids + 1):
        ...
        mask_1 = ((df_kid_kbz["monat_ad"] - df_kid_kbz[f"monat_ad_kind{id_kid}"]) >= 0) & (
        ...
        mask2 = ((df_kid_kbz["monat_ad"] - df_kid_kbz[f"monat_ad_kind{id_kid}"]) >= 0) & (
        ...
        count_kez_overlaps = count_kez_overlaps.reindex(df_kid_kez["rv_id"].unique(), fill_value=0)
        df_kid_kez["overlap_count"] = df_kid_kez["rv_id"].map(count_kez_overlaps)
        df_kid_kez["KEZ"] = df_kid_kez["KEZ"].replace(2, 1)
        mask2_adjusted = ((df_kid_kbz["monat_ad"] - df_kid_kbz[f"monat_ad_kind{id_kid}"]) >= 0) & (
                ((df_kid_kez[f"GBKIJ{id_kid}"] < 1992) & (
                        (df_kid_kez["monat_ad"] - df_kid_kez[f"monat_ad_kind{id_kid}"]) < 30 + df_kid_kez[
                    "overlap_count"])) | (
                        (df_kid_kez[f"GBKIJ{id_kid}"] >= 1992) & (
    ...
    df_var = pd.merge(df_var, df_kid_combo[merge_vars + ["KBZ", "KEZ"]], on=merge_vars, how="left")
...
def add_valid_alg_flag(df):
    ...
    check_2_year_window = (df["JAHR"].astype(float)*12 + df["MONAT"]).between(df["RB_months"] - 24,df["RB_months"])
...
def add_valid_fwb_flag(df):
    ...
    pflichtbeitraege_checklist = ['ALG', 'WSB', 'OSB', 'ATZ OSB', 'DDR OSB', 'DDR OKN', 'OKN', 'WSS',
                                  'ATZ WSB', 'OSS', 'WKN', 'ATZ OKN', 'BRF', 'PMU',
                                  'ARM', 'AUF', 'PFL', 'FRG BRF', 'NJB', 'USV',
                                  'FRG BSH', 'VRS', 'FRG ARM', 'BMP', 'KEZ']
    ...
    df["pflichtbeitrag"] = status_1_check | status_2_check | status_3_check | status_5_check | status_6_check
...
def count_valid_months(df):
    ...
    '''
    status_checklist = [
        'WSB', 'OSB', 'ATZ OSB', 'DDR OSB', 'DDR OKN', 'OKN', 'WSS',
        'ATZ WSB', 'OSS', 'WKN', 'ATZ OKN', 'BRF', 'PMU',
        'ARM', 'AUF', 'PFL', 'FRG BRF', 'NJB', 'USV',
        'FRG BSH', 'VRS', 'FRG ARM', 'BMP', 'KEZ', 'KBZ'
    ]
    ...
    fwb_check = fwb_month & df["FWB_valide"] & (
        (alg1_receiver & df["alg1_valide"]) | ~alg1_receiver
    )

    df["wartezeit_valide"] = (status_1_check | status_2_check | status_3_check | status_5_check
                              | status_6_check | df["alg1_valide"] | fwb_check)
    ...
    status_4_count = pd.Series(index=df["rv_id"].unique(), data=status_4_count).fillna(0)

    # add column for Wartezeit (in years) to df
    years_per_person = (df.groupby("rv_id")["wartezeit_valide"].sum() + status_4_count) / 12
...
def clean_up_df(df):
    ...
    '''
    extra_columns = ["KBZ", "KEZ", "alg1_valide", "pflichtbeitrag", "FWB_valide", "wartezeit_valide", "RB_months"]
    df = df.drop(columns = extra_columns)
    return df


df_final = load_var_df_and_create_status_6(data_path=data_path, berichtsjahr=berichtsjahr)
